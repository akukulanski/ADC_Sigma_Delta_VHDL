...
fileExec = 'd:/ivan/adc/codigo vhdl/CIC/testbench_files/binAscii_to_int16';
...
for fin=1000:1000:30000;
    ...
    out = fread(fd,'int16')
    fclose(fd);
    tit=sprintf('Frequency: %d Hz', fin);
...
for freq=[1000,10000,11000,15000];
    ...
    frewind(fd)
    %out = fread(fd,'int16')
    out_strv=fscanf(fd, '%16c\n')
    out_strm=vec2mat(out_strv,16);    
    fclose(fd);
    out=typecast(uint16(bin2dec(out_strm)),'int16');
    %typecast(int16(input(i)),'uint16')
    figure();
