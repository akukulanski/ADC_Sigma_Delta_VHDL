...
\title{Design, Implementation and Characterization of a Sigma-Delta Analog-to-Digital Converter in FPGA}
...
\begin{abstract}
%\boldmath
While analog to digital converters are often part of microcontrollers, this doesn't happen with FPGA, forcing a developer to obtain an external converter for his projects. As a FPGA is a widely used tool for DSP, we developed a Sigma-Delta ADC in the FPGA, using very low external hardware. In this publication we describe the design, implementation and measure of this converter.
...
Sigma-delta ($\Sigma\Delta$) analog to digital converter is based on two main conceps: \textit{Oversampling} and \textit{Noise Shaping}.
\textit{Oversampling} consists on sampling a signal with a frequency higher than Nyquist's. This distributes the quantization noise in a bigger bandwidth.
\textit{Noise shaping} consists on redistributing the quantization noise in such a way that most of it will be out of the band of interest.
Both concepts are shown in Fig.~\ref{fig:NS}.
The modulator is followed by a digital low pass decimation filter, eliminating most of the in-band quantization noise. This allows converters with high ENOB.

\begin{figure}[!b]
\centering
\includegraphics[width=1.7in]{Noise_Shaping.eps}
\caption{Up: Oversampling. Low: Oversampling+Noise Shapping}
\label{fig:NS}
\end{figure}

The $\Sigma\Delta$ ADC topology is in the block diagram of Fig. ~\ref{fig:SDTD}. The quantization noise is modeled as additive white noise ($N(z)$). The transfers functions for the signal (STF) and the noise (NTF) are:

\begin{align}
STF(z)=\frac{H(z)}{1+H(z)} \qquad
NTF(z)=\frac{1}{1+H(z)}
\end{align}

Being H a low pass filter, STF will be a low pass filter, and NTF will be a high pass filter, obtaining the previously mentioned Noise Shaping.

\begin{figure}[!t]
\centering
\includegraphics[height=1in]{Sigma-Delta_Tiempo_Discreto.eps}
\caption{Block diagram of the $\Sigma\Delta$}
\label{fig:SDTD}
\end{figure}

% % Poner en materiales y metodos
For the develop of the project, we used the developing kit \textit{Mojo v3}, that incorpores a \textit{Xilinx Spartan 6 FPGA}.
% %

In ~\ref{fig:DBADC} we show the basic blocks of the converter, where we can distinguish the modulator (analog stage), and two digital filters (CIC and FIR). The CIC (Cascaded integrator–comb) is a decimation filter that can be implemented with few%low
FPGA resources. The decimation factor we used is 512. The FIR is the one that will eliminate the noise accumulated in the high frequencies. After the FIR, we decimate again with a factor of 4, obtaining a total decimation factor of 2048.

\begin{figure}[!t]
\centering
\includegraphics[width=3.5in]{Diagrama_Sistema_ADC.eps}
\caption{Block diagram of the ADC}
\label{fig:DBADC}
\end{figure}

\subsection{Discrete-Time $\Sigma\Delta$ Modulator}

If H in Fig. 2 as an accumulator:

\begin{align}
H(z)=\frac{z^{-1}}{1-z^{-1}}
\end{align}

Using Eq. 1, we obtain the STF and NTF as:
...
Then the STF is just a delay, while the NTF is a high pass filter as is wanted.
To implement a k-order filter, the following NTF is used:
...
Assuming the quantization noise (QN) is white and uniformly distributed between -$\Delta/2$ and $\Delta/2$, being $\Delta$ the quantization step, it can be shown that:

% % poner una referencia

\begin{align}
S_q(f) &= \frac{\Delta^{2}}{12f_s}\\
P_{q}(f) &=\int\limits_{-f_s/2}^{f_s/2}S_q(f) \, |NTF(f) \times H_D(f)|^{2} df
\end{align}

where $S_q$ is the QN power spectral density and $P_q$ is the QN power after Noise Shapping and Digital Filtering, being $H_D$ the transfer function of the digital filter.
For an ideal digital filter with cutoff frequency FP, and a modulator of order k, the signal to quantization noise relationship is:
...
% %CAMBIAR FORMULA DE ARRIBA POR LA FORMULA APROXIMADA QUE APARECE SIEMPRE CON UNA REFERENCIA Y SIN ESTA EXPLICACION
%\footnote[1]
%{
...
The resulting SQNR is plotted in Fig.~\ref{fig:SQNR} as a function of the OSR for different filters orders.
...
\caption{SQNR as a function of de OSR for different filters orders}
\label{fig:SQNR}
\end{figure}
...
\section{Continuous-Time $\Sigma\Delta$ Modulator}


Unlike the discrete-time $\Sigma\Delta$ modulator, here the sampling is made inside the modulator, right after quantization (Fig.~\ref{fig:SDTC}). The STF and NTF are obtained as:

\begin{align}
STF(e^{sT_s}) &= \frac{H(s)}{1+H_{eq}(e^{sT_s})} \\
NTF(z) &= \frac{1}{1+H_{eq}(z)} \\
h_{eq}[n] &= (h\ast h_{DAC})(nT_s)
\end{align}

where the equivalent discrete-time $H_eq$ response is obtained with the impulse-invariance transform\cite{DSP:Pro-Man} of $H(s)H_{DAC}(s)$ defined in (11).
Using the DAC as a Zero-order hold is:
...
With the first order discrete time modulator and the previous transformation, we obtain:

\begin{align}
 H(s)&=\frac{1}{sT_s}\\
 STF(e^{sT_s})&= \frac{1-e^{-sT_s}}{s}
\end{align}

Being H an integrator, and the STF a low pass filter with zeros in multiples of $f_s$, which reduces the alias in the passband.

\begin{figure}[!t]
\centering
\includegraphics[height=1in]{Sigma-Delta_Tiempo_Continuo}
\caption{Block diagram of a continuous-time $\Sigma\Delta$ modulator}
\label{fig:SDTC}
\end{figure}

% %Seguir desde aca
%A. Implementation
% %The simplest implementation for the modulator is an RC filter (fig. 6). The quantization is done by a LVDS input of the FPGA [6], commonly used for differnetial communications, and the time discretization is done by a flip flop.
...
\begin{align}
H_{eq}(z)=\frac{z^{-1}}{2}\frac{1-e^{-\frac{2}{RC}T_s}}{1-e^{-\frac{2}{RC}T_s}z^{-1}}
\end{align}
