	...
	if(generalConfig.configure){
		...
		do{
			...
			if(cc=='p'){
				...
				readUntil(fileLoad);
				vector<short> vectorLoaded;
				Load();//vectorLoaded; //Load(File, myVector);
				send2Matlab(vectorLoaded, generalConfig, 0);//no uart
			}
		}while(cc=='p');
		string temp("");
		while(cc == 'e'){
			...
			//Autoplot
			cout << "Auto Sync Samples (0-1000) [" <<  generalConfig.syncSamples << "]: ";
			readUntil(temp);
...
int writeLog(fstream & logFile, sGeneralConfig generalConfig, sUartConfig uartConfig){
	...
	logFile << "binfilename=" << generalConfig.fileNameBin << endl;
...
int receiveUart(sGeneralConfig generalConfig, sUartConfig uartConfig, sFiles & Files, vector<short int> &myData){
	...
	send2Matlab(myData, generalConfig, uartConfig.cport_nr);
	return cant;
}
